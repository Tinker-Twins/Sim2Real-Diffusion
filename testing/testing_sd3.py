...
try:
    ...
    pipe = StableDiffusion3Img2ImgPipeline.from_pretrained(
        ...
        text_encoder_3=None,
        tokenizer_3=None,).to("cuda")

# Get the current directory where the script is located
script_dir = os.getcwd()

# Folder name to load/store images
image_folder = os.path.join(script_dir, "input_sd3") # Load input frames from here
style_folder = os.path.join(script_dir, "style_autodrive_large_offroad") # Load style frames from here
output_folder = os.path.join(script_dir, "output_sd3") # Store output results here
